...
export default function WebGLTextDistortion() {
  ...
  useEffect(() => {
    ...
    function createTextTexture() {
      ...
      const actualCharWidth = ctx.measureText('nope ').width / 5 // 5 characters in "nope "
      ...
      // Calculate exact number of "nope " blocks per line
      ...
      for (let lineIndex = 0; lineIndex < linesCount; lineIndex++) {
        ...
        if (!specialInserted && isInSpecialTextZone) {
          // Replace exactly 4 "nope " blocks instead of 5 to fix missing nope
          const nopesBeforeSpecial = nopesPerLine - 4 // Replace 4 nopes with special text
          ...
          // Add special text (replaces 4 nopes)
          line += specialText + ' '
          ...
          const hellPositionInSpecialText = 3 // "oh " is 3 characters, so "hell" starts at position 3
          const totalCharPosition = specialTextStartPos + hellPositionInSpecialText + 2 // +2 to center on "hell"
          ...
          // Regular lines: exactly nopesPerLine "nope " blocks
      ...
      if (!specialInserted) {
        ...
        // Updated fallback to also replace 4 nopes instead of 5
        const nopesBeforeSpecial = nopesPerLine - 4
        let line = ''
        ...
        const specialTextStartPos = nopesBeforeSpecial * text.length
        const hellPositionInSpecialText = 3
        const totalCharPosition = specialTextStartPos + hellPositionInSpecialText + 2
    ...
    function resizeCanvas() {
      ...
      const actualCharWidth = tempCtx.measureText('nope ').width / 5

...
If .gsd/ missing: Error — project not initialized.
</step>
...
<summary_creation>
After all tasks complete, create `{phase}-{plan}-SUMMARY.md` at `.gsd/phases/XX-name/`.
...
<final_commit>
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .gsd/phases/XX-name/{phase}-{plan}-SUMMARY.md .gsd/STATE.md
```

...
```bash
cat .gsd/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.cjs commit "fix($PHASE): revise plans based on checker feedback" --files .gsd/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
cat .gsd/phases/{selected-phase}/*-SUMMARY.md
```
...
Write to `.gsd/phases/XX-name/{phase}-{NN}-PLAN.md`
...
```bash
node ~/.claude/get-shit-done/bin/gsd-tools.cjs commit "docs($PHASE): create phase plan" --files .gsd/phases/$PHASE-*/$PHASE-*-PLAN.md .gsd/ROADMAP.md
```
</step>

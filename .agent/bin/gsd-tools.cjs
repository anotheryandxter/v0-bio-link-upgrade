 ...
 *   config-ensure-section              Initialize .gsd/config.json
 ...
 *   validate health [--repair]         Check .gsd/ integrity, optionally repair
 *
 * Progress:
...
function loadConfig(cwd) {
  const configPath = path.join(cwd, '.gsd', 'config.json');
  const defaults = {
...
function cmdListTodos(cwd, area, raw) {
  const pendingDir = path.join(cwd, '.gsd', 'todos', 'pending');
    ...
    for (const file of files) {
      try {
        ...
        todos.push({
          ...
          area: todoArea,
          path: path.join('.gsd', 'todos', 'pending', file),
        });
      } catch {}
    }
  } catch {}
...
function cmdConfigEnsureSection(cwd, raw) {
  const configPath = path.join(cwd, '.gsd', 'config.json');
  const planningDir = path.join(cwd, '.gsd');
  ...
  try {
    ...
    error('Failed to create .gsd directory: ' + err.message);
  }
  ...
  try {
    ...
    const result = { created: true, path: '.gsd/config.json' };
...
function cmdHistoryDigest(cwd, raw) {
  const phasesDir = path.join(cwd, '.gsd', 'phases');
...
function cmdRoadmapGetPhase(cwd, phaseNum, raw) {
  const roadmapPath = path.join(cwd, '.gsd', 'ROADMAP.md');
...
function cmdFindPhase(cwd, phase, raw) {
  ...
  try {
    ...
    const result = {
      found: true,
      directory: path.join('.gsd', 'phases', match),
...
function cmdCommit(cwd, message, files, raw, amend) {
  ...
  // Check if .gsd is gitignored
  if (isGitIgnored(cwd, '.gsd')) {
  ...
  // Stage files
  const filesToStage = files && files.length > 0 ? files : ['.gsd/'];
...
function cmdPhaseAdd(cwd, description, raw) {
  ...
  const dirPath = path.join(cwd, '.gsd', 'phases', dirName);
...
function cmdPhaseComplete(cwd, phaseNum, raw) {
  ...
  if (fs.existsSync(roadmapPath)) {
    ...
    const reqPath = path.join(cwd, '.gsd', 'REQUIREMENTS.md');
...
function cmdMilestoneComplete(cwd, version, options, raw) {
  ...
  const reqPath = path.join(cwd, '.gsd', 'REQUIREMENTS.md');
  const statePath = path.join(cwd, '.gsd', 'STATE.md');
  const milestonesPath = path.join(cwd, '.gsd', 'MILESTONES.md');
  const archiveDir = path.join(cwd, '.gsd', 'milestones');
  const phasesDir = path.join(cwd, '.gsd', 'phases');
  const today = new Date().toISOString().split('T')[0];
  ...
  if (fs.existsSync(reqPath)) {
    ...
    const archiveHeader = `# Requirements Archive: ${version} ${milestoneName}\n\n**Archived:** ${today}\n**Status:** SHIPPED\n\nFor current requirements, see \`.gsd/REQUIREMENTS.md\`.\n\n---\n\n`;
    fs.writeFileSync(path.join(archiveDir, `${version}-REQUIREMENTS.md`), archiveHeader + reqContent, 'utf-8');
  }

  // Archive audit file if exists
  const auditFile = path.join(cwd, '.gsd', `${version}-MILESTONE-AUDIT.md`);
...
function cmdValidateHealth(cwd, options, raw) {
  ...
  // ─── Check 1: .gsd/ exists ───────────────────────────────────────────
  if (!fs.existsSync(planningDir)) {
    addIssue('error', 'E001', '.gsd/ directory not found', 'Run /gsd:new-project to initialize');
    output({
      status: 'broken',
      errors,
      warnings,
      ...
      try {
        switch (repair) {
          ...
          case 'regenerateState': {
            ...
            stateContent += `See: .gsd/PROJECT.md\n\n`;
...
function cmdTodoComplete(cwd, filename, raw) {
  ...
  const pendingDir = path.join(cwd, '.gsd', 'todos', 'pending');
  const completedDir = path.join(cwd, '.gsd', 'todos', 'completed');
...
function cmdScaffold(cwd, type, options, raw) {
  ...
  switch (type) {
    ...
    case 'phase-dir': {
      ...
      const phasesParent = path.join(cwd, '.gsd', 'phases');
      fs.mkdirSync(phasesParent, { recursive: true });
      const dirPath = path.join(phasesParent, dirName);
      fs.mkdirSync(dirPath, { recursive: true });
      output({ created: true, directory: `.gsd/phases/${dirName}`, path: dirPath }, raw, dirPath);
      return;
    }
    default:
    ...
    for (const archiveName of phaseDirs) {
      ...
      for (const dir of dirs) {
        results.push({
          name: dir,
          milestone: version,
          basePath: path.join('.gsd', 'milestones', archiveName),
...
function findPhaseInternal(cwd, phase) {
  ...
  const current = searchPhaseInDir(phasesDir, path.join('.gsd', 'phases'), normalized);
  ...
  try {
    ...
    for (const archiveName of archiveDirs) {
      const version = archiveName.match(/^(v[\d.]+)-phases$/)[1];
      const archivePath = path.join(milestonesDir, archiveName);
      const relBase = path.join('.gsd', 'milestones', archiveName);
...
function getMilestoneInfo(cwd) {
  try {
    const roadmap = fs.readFileSync(path.join(cwd, '.gsd', 'ROADMAP.md'), 'utf-8');
...
function cmdInitExecutePhase(cwd, phase, includes, raw) {
  ...
  const result = {
    ...
    // File existence
    state_exists: pathExistsInternal(cwd, '.gsd/STATE.md'),
    roadmap_exists: pathExistsInternal(cwd, '.gsd/ROADMAP.md'),
    config_exists: pathExistsInternal(cwd, '.gsd/config.json'),
  };

  // Include file contents if requested via --include
  if (includes.has('state')) {
    result.state_content = safeReadFile(path.join(cwd, '.gsd', 'STATE.md'));
  }
  if (includes.has('config')) {
    result.config_content = safeReadFile(path.join(cwd, '.gsd', 'config.json'));
  }
  if (includes.has('roadmap')) {
    result.roadmap_content = safeReadFile(path.join(cwd, '.gsd', 'ROADMAP.md'));
  }
...
function cmdInitPlanPhase(cwd, phase, includes, raw) {
  ...
  const result = {
    ...
    // Environment
    planning_exists: pathExistsInternal(cwd, '.gsd'),
    roadmap_exists: pathExistsInternal(cwd, '.gsd/ROADMAP.md'),
  };
  ...
  if (includes.has('requirements')) {
    result.requirements_content = safeReadFile(path.join(cwd, '.gsd', 'REQUIREMENTS.md'));
  }
...
function cmdInitNewProject(cwd, raw) {
  ...
  const result = {
    ...
    // Existing state
    project_exists: pathExistsInternal(cwd, '.gsd/PROJECT.md'),
    has_codebase_map: pathExistsInternal(cwd, '.gsd/codebase'),
    planning_exists: pathExistsInternal(cwd, '.gsd'),
    ...
    needs_codebase_map: (hasCode || hasPackageFile) && !pathExistsInternal(cwd, '.gsd/codebase'),
...
function cmdInitQuick(cwd, description, raw) {
  ...
  const quickDir = path.join(cwd, '.gsd', 'quick');
  let nextNum = 1;
  try {
  ...
  const result = {
    ...
    // Paths
    quick_dir: '.gsd/quick',
    task_dir: slug ? `.gsd/quick/${nextNum}-${slug}` : null,
...
function cmdInitResume(cwd, raw) {
  ...
  try {
    interruptedAgentId = fs.readFileSync(path.join(cwd, '.gsd', 'current-agent-id.txt'), 'utf-8').trim();
  } catch {}
...
function cmdInitTodos(cwd, area, raw) {
  ...
  const result = {
    ...
    // Paths
    pending_dir: '.gsd/todos/pending',
    completed_dir: '.gsd/todos/completed',

    // File existence
    planning_exists: pathExistsInternal(cwd, '.gsd'),
    todos_dir_exists: pathExistsInternal(cwd, '.gsd/todos'),
    pending_dir_exists: pathExistsInternal(cwd, '.gsd/todos/pending'),
  };
...
function cmdInitMilestoneOp(cwd, raw) {
  ...
  // Check archive
  const archiveDir = path.join(cwd, '.gsd', 'archive');
  ...
  const result = {
    ...
    roadmap_exists: pathExistsInternal(cwd, '.gsd/ROADMAP.md'),
    state_exists: pathExistsInternal(cwd, '.gsd/STATE.md'),
    archive_exists: pathExistsInternal(cwd, '.gsd/archive'),
    phases_dir_exists: pathExistsInternal(cwd, '.gsd/phases'),
  };
...
function cmdInitMapCodebase(cwd, raw) {
  ...
  const codebaseDir = path.join(cwd, '.gsd', 'codebase');
  let existingMaps = [];
  try {
  ...
  const result = {
    ...
    // File existence
    planning_exists: pathExistsInternal(cwd, '.gsd'),
    codebase_dir_exists: pathExistsInternal(cwd, '.gsd/codebase'),
  };
...
function cmdInitProgress(cwd, includes, raw) {
  ...
  try {
    ...
    for (const dir of dirs) {
      ...
      const phaseInfo = {
        number: phaseNumber,
        name: phaseName,
        directory: path.join('.gsd', 'phases', dir),
        status,
        plan_count: plans.length,
        summary_count: summaries.length,
        has_research: hasResearch,
  ...
  let pausedAt = null;
  try {
    const state = fs.readFileSync(path.join(cwd, '.gsd', 'STATE.md'), 'utf-8');
  ...
  if (includes.has('project')) {
    result.project_content = safeReadFile(path.join(cwd, '.gsd', 'PROJECT.md'));
  }
  if (includes.has('config')) {
    result.config_content = safeReadFile(path.join(cwd, '.gsd', 'config.json'));
  }

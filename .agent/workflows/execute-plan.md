...
## 1. Init Context" priority="first
Load execution context (uses `init execute-phase` for full context, including file contents):

```bash
INIT=$(node .agent/bin/gsd-tools.cjs init execute-phase "${PHASE}" --include state,config)
```
...
## 8. Previous Phase Check
```bash
node .agent/bin/gsd-tools.cjs phases list --type summaries --raw
...
## 11. Checkpoint Return For Orchestrator
...
## 15. Create Summary
Create `{phase}-{plan}-SUMMARY.md` at `.gsd/phases/XX-name/`. Use `~/.claude/get-shit-done/templates/summary.md`.
...
node .agent/bin/gsd-tools.cjs state advance-plan

# Recalculate progress bar from disk state
node .agent/bin/gsd-tools.cjs state update-progress

# Record execution metrics
node .agent/bin/gsd-tools.cjs state record-metric \
  --phase "${PHASE}" --plan "${PLAN}" --duration "${DURATION}" \
  --tasks "${TASK_COUNT}" --files "${FILE_COUNT}"
```


## 17. Extract Decisions And Issues
From SUMMARY: Extract decisions and add to STATE.md:

```bash
# Add each decision from SUMMARY key-decisions
node .agent/bin/gsd-tools.cjs state add-decision \
  --phase "${PHASE}" --summary "${DECISION_TEXT}" --rationale "${RATIONALE}"

# Add blockers if any found
node .agent/bin/gsd-tools.cjs state add-blocker "Blocker description"
```


## 18. Update Session Continuity
Update session info using gsd-tools:

```bash
node .agent/bin/gsd-tools.cjs state record-session \
...
## 19. Issues Review Gate
If SUMMARY "Issues Encountered" ≠ "None": yolo → log and continue. Interactive → present issues, wait for acknowledgment.


## 20. Update Roadmap
```bash
node .agent/bin/gsd-tools.cjs roadmap update-plan-progress "${PHASE}"
```
Counts PLAN vs SUMMARY files on disk. Updates progress table row with correct count and status (`In Progress` or `Complete` with date).


## 21. Git Commit Metadata
Task code already committed per-task. Commit plan metadata:

```bash
node .agent/bin/gsd-tools.cjs commit "docs({phase}-{plan}): complete [plan-name] plan" --files .gsd/phases/XX-name/{phase}-{plan}-SUMMARY.md .gsd/STATE.md .gsd/ROADMAP.md
```


## 22. Update Codebase Map
If .gsd/codebase/ doesn't exist: skip.
...
```bash
node .agent/bin/gsd-tools.cjs commit "" --files .gsd/codebase/*.md --amend
```
...
|-----------|-------|--------|
| summaries < plans | **A: More plans** | Find next PLAN without SUMMARY. Yolo: auto-continue. Interactive: show next plan, suggest `/execute-phase {phase}` + `/verify-work`. STOP here. |
| summaries = plans, current < highest phase | **B: Phase done** | Show completion, suggest `/plan-phase {Z+1}` + `/verify-work {Z}` + `/discuss-phase {Z+1}` |
| summaries = plans, current = highest phase | **C: Milestone done** | Show banner, suggest `/complete-milestone` + `/verify-work` + `/add-phase` |

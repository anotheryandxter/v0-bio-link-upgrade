---
description: Mark current phase complete and advance to next. This is the natural point where progress tracking and PROJECT.md evolution happen.
...
## 1. Load Project State" priority="first
...
```bash
TRANSITION=$(node .agent/bin/gsd-tools.cjs phase complete "${current_phase}")
```
...
## 7. Update Current Position After Transition
...
```bash
PROGRESS=$(node .agent/bin/gsd-tools.cjs progress bar --raw)
```
...
## 10. Update Session Continuity After Transition
...
```bash
ROADMAP=$(node .agent/bin/gsd-tools.cjs roadmap analyze)
```
...
```bash
ls .gsd/phases/*[X+1]*/*-CONTEXT.md 2>/dev/null
```
...
Exit skill and invoke SlashCommand("/plan-phase [X+1] --auto")
...
Exit skill and invoke SlashCommand("/discuss-phase [X+1] --auto")
...
`/discuss-phase [X+1]` — gather context and clarify approach
...
**Also available:**
- `/plan-phase [X+1]` — skip discussion, plan directly
- `/research-phase [X+1]` — investigate unknowns
...
**Also available:**
- `/discuss-phase [X+1]` — revisit context
- `/research-phase [X+1]` — investigate unknowns
...
```bash
node .agent/bin/gsd-tools.cjs config-set workflow.auto_advance false
```

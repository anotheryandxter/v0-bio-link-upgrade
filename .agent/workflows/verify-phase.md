---
description: Verify phase goal achievement through goal-backward analysis. Check that the codebase delivers what the phase promised, not just that tasks completed.
...
## 1. Load Context" priority="first
Load phase operation context:

```bash
INIT=$(node .agent/bin/gsd-tools.cjs init phase-op "${PHASE_ARG}")
```
...
```bash
node .agent/bin/gsd-tools.cjs roadmap get-phase "${phase_number}"
grep -E "^| ${phase_number}" .gsd/REQUIREMENTS.md 2>/dev/null
...
for plan in "$PHASE_DIR"/*-PLAN.md; do
  MUST_HAVES=$(node .agent/bin/gsd-tools.cjs frontmatter get "$plan" --field must_haves)
...
```bash
PHASE_DATA=$(node .agent/bin/gsd-tools.cjs roadmap get-phase "${phase_number}" --raw)
```
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  ARTIFACT_RESULT=$(node .agent/bin/gsd-tools.cjs verify artifacts "$plan")
...
```bash
for plan in "$PHASE_DIR"/*-PLAN.md; do
  LINKS_RESULT=$(node .agent/bin/gsd-tools.cjs verify key-links "$plan")

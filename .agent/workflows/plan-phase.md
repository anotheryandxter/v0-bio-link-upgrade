---
description: Create executable phase prompts (PLAN.md files) for a roadmap phase with integrated research and verification. Default flow: Research (if needed) -> Plan -> Verify -> Done. Orchestrates gsd-phase-researcher, gsd-planner, and gsd-plan-checker agents with a revision loop (max 3 iterations).
...
```bash
INIT_RAW=$(node .agent/bin/gsd-tools.cjs init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
...
```bash
mkdir -p ".gsd/phases/${padded_phase}-${phase_slug}"
```
...
```bash
PHASE_INFO=$(node .agent/bin/gsd-tools.cjs roadmap get-phase "${PHASE}")
```
...
If "Run discuss-phase first": Display `/discuss-phase {X}` and exit workflow.
...
```bash
PHASE_DESC=$(node .agent/bin/gsd-tools.cjs roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node .agent/bin/gsd-tools.cjs state-snapshot)
...
<phase_context>
IMPORTANT: If CONTEXT.md exists below, it contains user decisions from /discuss-phase.
...
**Phase Context:**
IMPORTANT: If context exists below, it contains USER DECISIONS from /discuss-phase.
...
```
Task(
  prompt="Run /execute-phase ${PHASE} --auto",
  subagent_type="general-purpose",
...
- **PHASE COMPLETE** → Display final summary:
  ...
  Next: /discuss-phase ${NEXT_PHASE} --auto
  ```
...
- /plan-phase {X} --research — re-research first

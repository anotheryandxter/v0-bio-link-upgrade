---
description: Orchestrate parallel codebase mapper agents to analyze codebase and produce structured documents in .gsd/codebase/
...
```bash
INIT=$(node .agent/bin/gsd-tools.cjs init map-codebase)
```
...
## 2. Check Existing
Check if .gsd/codebase/ already exists using `has_maps` from init context.
...
grep -E '(sk-[a-zA-Z0-9]{20,}|sk_live_[a-zA-Z0-9]+|sk_test_[a-zA-Z0-9]+|ghp_[a-zA-Z0-9]{36}|gho_[a-zA-Z0-9]{36}|glpat-[a-zA-Z0-9_-]+|AKIA[A-Z0-9]{16}|xox[baprs]-[a-zA-Z0-9-]+|-----BEGIN.*PRIVATE KEY|eyJ[a-zA-Z0-9_-]+\.eyJ[a-zA-Z0-9_-]+\.)' .gsd/codebase/*.md 2>/dev/null && SECRETS_FOUND=true || SECRETS_FOUND=false
```
...
```bash
node .agent/bin/gsd-tools.cjs commit "docs: map existing codebase" --files .gsd/codebase/*.md
```
